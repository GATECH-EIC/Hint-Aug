...
from engine_patch import train_one_epoch, evaluate
from lib.samplers import RASampler
from lib import utils
from lib.config import cfg, update_config_from_file
from model.supernet_vision_transformer_timm_patch import VisionTransformer
from model.supernet_vision_transformer_timm import vit_base_patch16_224_in21k
from model.supernet_vision_transformer_timm_switch import switchableNorm
...
def get_args_parser():
    ...
    parser.add_argument('--model', default='', type=str, metavar='MODEL',
                        help='Name of model to train')
    # AutoFormer config
    parser.add_argument('--mode', type=str, default='super', choices=['super', 'vp','retrain','search', 'baseline'], help='mode of AutoFormer')
    ...
    parser.add_argument('--atk_type', default='untargeted', type=str)

    parser.add_argument('--switch_bn', default=False, type=bool, help='use switchable bn')

    # Patch-fool params
    parser.add_argument('--patch_fool',action='store_true')
    ...
    parser.add_argument('--attack_learning_rate', default=0.1, type=float)
    parser.add_argument('--step_size', default=5, type=int)
    parser.add_argument('--gamma', default=0.95, type=float)

    parser.add_argument('--use_cf_mat',action='store_true')
    parser.add_argument('--use_pre_soft',action='store_true')
    parser.add_argument('--transmix', action='store_true')
    parser.add_argument('--start_adv', default=20, type=int)
...
def main(args):
    ...
    data_loader_val = torch.utils.data.DataLoader(
        dataset_val, batch_size=int(3 * args.batch_size),
    ...
    if args.switch_bn:
        # print('*'*100)
        print('using switchable bn')
        utils.replace_bn(model, torch.nn.LayerNorm, switchableNorm)
        # model_module = unwrap_model(model)

    gen_model = models.__dict__[cfg.MODEL_NAME](   
                                                img_size=args.input_size,
                                                drop_rate=args.drop,
                                                drop_path_rate=args.drop_path,
                                                super_prompt_tuning_dim=cfg.SUPERNET.VISUAL_PROMPT_DIM,super_LoRA_dim=cfg.SUPERNET.LORA_DIM,super_adapter_dim=cfg.SUPERNET.ADAPTER_DIM,super_prefix_dim=cfg.SUPERNET.PREFIX_DIM,
    ...
    if args.teacher_model:
        teacher_model = vit_base_patch16_224_in21k(pretrained=True)
    ...
    if args.distributed:
        ...
        gen_model = torch.nn.parallel.DistributedDataParallel(gen_model, device_ids=[args.gpu], find_unused_parameters=True)
        gen_model_without_ddp = gen_model.module
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device,  mode = args.mode, retrain_config=retrain_config,is_visual_prompt_tuning=args.is_visual_prompt_tuning,is_adapter=args.is_adapter,is_LoRA=args.is_LoRA,is_prefix=args.is_prefix, patch_fool=args.patch_fool)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, train_pred, train_true, train_out = train_one_epoch(
            ...
            is_visual_prompt_tuning=args.is_visual_prompt_tuning,is_adapter=args.is_adapter,is_LoRA=args.is_LoRA,is_prefix=args.is_prefix, classes=args.nb_classes, gen_model=gen_model, cf_matrix=cf_mat, eps=args.eps, args=args, pre_soft_mat=pre_soft_mat
        )
        train_true = np.array(train_true)
        train_true = torch.from_numpy(train_true).cuda()
        all_true = [torch.zeros_like(train_true) for i in range(4)]
        torch.distributed.all_gather(all_true, train_true)

        train_out = np.array(train_out)
        train_out = torch.from_numpy(train_out).cuda()
        all_out = [torch.zeros_like(train_out) for i in range(4)]
        torch.distributed.all_gather(all_out, train_out)
        if args.use_pre_soft:
            pre_soft_mat = torch.zeros((args.nb_classes, args.nb_classes))
            for gpu_out, gpu_true in zip(all_out, all_true):
                for out, true in zip(gpu_out, gpu_true):
                    out = out.detach().cpu()
                    out = out - out.min()
                    pre_soft_mat[true] += out.detach().cpu()
        ...
        # if epoch > 1:
        if epoch > 50 and epoch % args.val_interval == 0 or epoch == args.epochs-1:
            test_stats, y_pred, y_true, y_out = evaluate(data_loader_val, model, device, amp=args.amp, choices=choices, mode = args.mode, retrain_config=retrain_config,is_visual_prompt_tuning=args.is_visual_prompt_tuning,is_adapter=args.is_adapter,is_LoRA=args.is_LoRA,is_prefix=args.is_prefix, patch_fool=args.patch_fool)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
