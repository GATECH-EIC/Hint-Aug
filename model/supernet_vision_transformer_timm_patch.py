...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.patch_fool:
            ...
            return x
    def get_attn(self, set_flag):
        self.patch_fool = set_flag 

class switchableNorm(nn.LayerNorm):
    def __init__(self, embedding_dim):
        super(switchableNorm, self).__init__(embedding_dim)
        
        self.embedding_dim = embedding_dim

        self.norm = nn.ModuleList(
            [nn.LayerNorm(embedding_dim), nn.LayerNorm(embedding_dim)]
        )
        self.flag = 0
    ...
    def norm_init(self):
        pass
        # self.norm[0].weight = self.weight
        # self.norm[0].bias = 

    def forward(self, input):
        # print('forward with flag {}'.format(self.flag))
        y = self.norm[self.flag](input)
        return y
...
@register_model
def vit_tiny_patch16_224(pretrained=False, patch_fool=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)
    model = _create_vision_transformer('vit_tiny_patch16_224', pretrained=pretrained, patch_fool=patch_fool, **model_kwargs)
    return model
...
@register_model
def vit_base_patch32_224_in21k(pretrained=False, patch_fool=False, **kwargs):
    """ ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = _create_vision_transformer('vit_base_patch32_224_in21k', pretrained=pretrained, patch_fool=patch_fool, **model_kwargs)
    return model


@register_model
def vit_base_patch16_224_in21k_patch(pretrained=False, patch_fool=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, patch_fool=patch_fool, **model_kwargs)
    return model
